...
- **CRITICAL**: When the version is updated, search for and update the `version` field in the following files to match:
  - `package.json` in the root directory.
  - `package.json` in sub-applications (e.g., `apps/client`, `apps/server`, `apps/shared`).
  - Any other manifested version hooks defined by the project.
...
- Write the changes in **Chinese** if the project is predominantly using Chinese or if the prompt is in Chinese, else default to English.
...
- **Automatically update the package version numbers** alongside the `CHANGELOG.md` update, ensuring the entire codebase reflects the release.
